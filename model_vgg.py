...
class VGG_16(nn.Module):
    ...
    def __init__(self, in_chan, streams, step, no_classes=0,  pretrain = True):
        ...
        #self.classifier = nn.ModuleList([nn.Linear(128,self.step)] for i in range(self.streams))
        ...
        self.f = nn.ModuleList(self.final)         
    ...
    def forward(self, x, strm=0):
        ...
        if(strm != 1 and strm != 0):
         ...
         for i in range (strm-1):
            temp = self.f[i](m(rep1))
            dist.append(temp)
         out_dist = torch.cat(dist, dim=1)
         for j in range(strm):
            temp = self.f[j](m(rep1))
            value.append(temp)
